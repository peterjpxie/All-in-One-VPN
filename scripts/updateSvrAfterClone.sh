...
# Need to check /etc/openvpn/server.conf next time
...
sed -i".old-$(date +%F-%T)" "s/$old_server_internal_ip/$new_server_internal_ip/" /etc/ipsec.conf
sed -i".old-$(date +%F-%T)" "s/$old_server_internal_ip/$new_server_internal_ip/" /etc/iptables.rules
sed -i".old-$(date +%F-%T)" "s/$old_server_internal_ip/$new_server_internal_ip/" /etc/rc.local
